...
//				.got_page("main order detail")
			...
			.request("submit order payment options").with_form("PaymentOptionSelection")
				...
				.when("paid success").comments("直接支付成功,例如全用余额支付了")
					.got_page("order paid success").title("支付成功")
					.comments("支付成功页面")
			...
			.request("payment callback").with_string("order id").with_string("payment id")
				.comments("支付交互完成后, 回调系统接口").need_login().no_footprint()
				.when("paid").comments("完成支付了")
					.got_page("order paid success")
						.comments("支付完成页面")
				.when_others().comments("其他未完成支付的任何情况")
					.got_page("payment info").title("支付未完成")
				;
	}
