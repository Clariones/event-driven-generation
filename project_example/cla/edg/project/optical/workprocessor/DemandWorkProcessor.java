package cla.edg.project.optical.workprocessor;
    ...
    public WorkProcessorBuilder makeSequel(WorkProcessorBuilder builder) {
        return builder.processing("demand proposal").zh_CN("需求处理流程")
            ...
            .in_status("wait supplier connect").zh_CN("等待供应商对接")
                ...
                .as_role("supplier")
                    .can_do("supplier confirm connect")
            ...
            .in_status("connected")
                .can_enter_when()
                    .must_have("wait user connect").all_success()
                    .must_have("wait supplier connect").all_success()

                .once_enter()
                    .split_to("wait_user_deal", "wait_supplier_deal")

            .in_status("wait_user_deal")
                .on_event("confirm").by_role("user").go_to("deal")
            ...
            .in_status("deal").zh_CN("已成交")
                .can_enter_when()
                    .must_have("wait user deal").all_success()
                    .must_have("wait supplier deal").all_success()

                .on_event("platform finalize")
                    .when("agree").zh_CN("协商一致").go_to("closed")
