		...
		return builder
			...
			.change_request("create contract").zh_CN("创建合同")
				.icon("form")
					...
					.has_field("important rules").zh_CN("重要条款").which_type_of(FieldType.MULTI_TEXT)
...
//					.has_field("contract document").zh_CN("合同文档").which_type_of(FieldType.of(""))
				.step("pay item").zh_CN("付款方式")
					.contains_event("contract payment phase").zh_CN("付款周期").many_times(1,100)
						...
						.has_field("payer").zh_CN("付款方").which_model_of(MODEL.merchant()).fill_by_submitted("party a")
						.has_field("payee").zh_CN("收款方").which_model_of(MODEL.merchant()).fill_by_submitted("party b")
