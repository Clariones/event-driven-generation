		...
		return builder
			...
			.change_request("create contract").zh_CN("创建合同")
				.icon("form")
					...
					.has_field("number").zh_CN("合同编号").which_type_of(FieldType.MULTI_TEXT).place_holder("请参照合同原本输入")
					.has_field("contract type").zh_CN("合同类型").disabled().fill_by_request("type id",MODEL.contractType().name())
					.has_field("status").zh_CN("状态").which_model_of(MODEL.contractStatus()).values_can_select_from_query_by(null)
					.has_field("project name").zh_CN("工程名称").disabled().fill_by_request("project id",MODEL.project().title())
					.has_field("project location").zh_CN("工程地址").disabled().fill_by_request("project id",MODEL.project().location())
...
//					.has_field("contract document").zh_CN("合同文档").which_type_of(FieldType.of(""))
				.step("pay item").zh_CN("付款方式")
					.contains_event("contract payment phase").zh_CN("付款周期").many_times(1,100).show_them_all_in_cr()
						...
						.has_field("payer").zh_CN("付款方").fill_by_submitted("party a", MODEL.merchant().name()).disabled()
						.has_field("payee").zh_CN("收款方").fill_by_submitted("party b", MODEL.merchant().name()).disabled()
			...
			.change_request("create additional contract").zh_CN("签证及补充合同")
				.icon("form")
					...
					.has_field("main contract").zh_CN("主合同").disabled().fill_by_request("contract id",MODEL.standardContract().name())
					.has_field("project name").zh_CN("工程名称").disabled().fill_by_request("project id",MODEL.project().title())
