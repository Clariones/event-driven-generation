...
public class WxappService extends BasePageFlowDescriptionScript {
	...
	private static final BasePageFlowScript SCRIPT = $("wxapp service").need_login().no_footprint()
		...
		.request("fill weight to order").with_string("scale id").with_float("weight").with_string("order id")
		...
		.request("new seller warehouse to order").with_string("scale id").with_float("weight").with_string("mobile")
			...
			.got_page("order detail")
		.request("fill seller warehouse to order").with_string("scale id").with_float("weight").with_string("warehouse id")
...
//		.request("go back to tare weigh").with_string("order id")
		.request("scan to weight with order").with_string("order id").with_string("weigh type")
			...
			.got_page("weigh with order")
				.comments("为指定的订单称重.")
				.may_request("submit weight with order")
		.request("submit weight with order").with_string("order id").with_string("weigh type").with_string("scale id").variable().with_float("weight").variable()
			.comments("提交称重结果")
			.got_page("order detail")
