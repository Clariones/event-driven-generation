			...
			.request("submit artwork auction application").with_form("artwork auction")
				...
				.when("lack of deposit").comments("保证金不足以支持该商品的起拍价")
					.got_page("adjust price or recharge deposit pop up").comments("选择‘调价’还是‘补保’的弹出页面")
						.may_request("pay artwork auction and deposit")
				.when_others().comments("保证金足够")
					.got_page("pay artwork auction")
			.request("pay artwork auction and deposit").with_form("artwork auction")
				.comments("保证金不足的情况下，选择补足保证金并上架商品")
				.got_page("pay artwork auction").comments("支付商品上架销售费用")
					.may_request("pay bill or order")
...
//				.comments("去支付订单或账单")
//				.when("still").comments("需要使用数字资产支付（墨贝，余额）")
