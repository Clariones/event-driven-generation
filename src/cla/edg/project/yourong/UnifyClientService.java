...
public class UnifyClientService extends BasePageFlowDescriptionScript {
	...
	private static final PageFlowScript SCRIPT = $("unify client service").need_login().has_footprint()
		...
		.query("operating_domain_introduction").which("all").no_pagination()
			.comments("查询中台配置的所有 运营领域介绍 条目")
		/** 项目相关的查询 */
		.query("project").which("user by status").pagination().with_string("filter").with_string("owner type").with_integer("top n")
