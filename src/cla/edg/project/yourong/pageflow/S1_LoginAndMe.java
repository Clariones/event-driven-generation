	...
	public PageFlowScript makeSequel(PageFlowScript script) {
		return script
			...
			.request("view account detail").with_string("account book id")
				.comments("查看账本,如果没有账目,就查看流水").need_login().no_footprint().can_refresh()
				.got_page("account detail")
					.may_request("view more account book items")
					.may_request("view running account record")
			.request("view account items").with_string("account book id")
				.comments("查看账本,固定查看账目").need_login().no_footprint().can_refresh()
				.got_page("account detail")
			...
			.request("view running account record").with_string("account book id")
				.comments("查看账本,固定查看流水").need_login().no_footprint()
				.got_page("account detail")
