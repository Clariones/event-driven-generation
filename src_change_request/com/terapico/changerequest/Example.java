				...
				.change_request("marriage registration").zh_CN("婚姻注册申请")
					...
					.step3("registration").zh_CN("注册信息")
						...
						// 同样的道理, 只有一个step可以向 changeRequest 取默认值
							.has_field("register date").as("申请注册日期").which_type_of(FieldType.DATE)
								.tips_title("?").tips_content("领取结婚证的日期")
								.place_holder("请选择申请注册的日期")
							.has_field("register provice").as("注册地-省")
								.tips_title("?").tips_content("前往领取结婚证的民政局所在地址")
								.values_can_get_from("getProvices/")
							.has_field("register city").as("注册地-市")
								.tips_title("?").tips_content("前往领取结婚证的民政局所在地址")
								.values_can_get_from("getCitiesByProvice/${register provice}/")
							.has_field("register office").as("注册地-民政局")
