...
<#assign login_target_model=loginInfo.userModelName/>
    ...
    protected void throwsExceptionWithMessage(${custom_context_name} ctx, String message) throws ${NAMING.toCamelCase(project_name)}Exception {
    	throw new ${NAMING.toCamelCase(project_name)}Exception(message);
    }
